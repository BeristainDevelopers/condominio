...
export const Navbar = () => {
    ...
    return (
        <>
        {/* NAV DESKTOP */}
        <nav className="bg-gray-300 p-6 shadow-md border-b border-gray-200 hidden lg:block">
        ...
        {/* NAV MOBILE */}
        <nav className="bg-gray-300 p-4 shadow-md border-b border-gray-200 flex justify-between items-center lg:hidden">
            <div className="text-lg font-bold">
                <Link to="/home">
                    <h1>LOGO</h1>
                </Link>
            ...
            {menuOpen && (
                <div className="absolute top-16 left-0 w-full bg-gray-300 shadow-md z-50">
                    <ul className="flex flex-col pl-4 py-4 gap-4">
                        <li>
                            <Link to="/" onClick={() => setMenuOpen(false)} className={`text-lg md:text-2xl ${location.pathname === "/" ? "text-indigo-700 font-bold text-xl" : "text-gray-700 font-semibold"}`}>
                                <FaHome className="inline-block mr-1" /> Inicio
                            </Link>
                        </li>
                        <li>
                            <Link to="/gastos-comunes" onClick={() => setMenuOpen(false)} className={`text-lg md:text-2xl ${location.pathname.startsWith("/gastos-comunes") ? "text-indigo-700 font-bold text-xl" : "text-gray-700 font-semibold"}`}>
                                <BsCashCoin className="inline-block mr-1" /> Gastos-Comunes
                            </Link>
                        </li>
                        <li>
                            <Link to="/anuncios" onClick={() => setMenuOpen(false)} className={`text-lg md:text-2xl ${location.pathname.startsWith("/anuncios") ? "text-indigo-700 font-bold text-xl" : "text-gray-700 font-semibold"}`}>
                                <FaRegNewspaper className="inline-block mr-1" /> Anuncios
                            </Link>
                        </li>
                        <li>
                            <Link to="/residentes" onClick={() => setMenuOpen(false)} className={`text-lg md:text-2xl ${location.pathname.startsWith("/residentes") ? "text-indigo-700 font-bold text-xl" : "text-gray-700 font-semibold"}`}>
                                <FaPeopleGroup className="inline-block mr-1" /> Residentes
                            </Link>
                        </li>
                        <li>
                            <Link to="/documentos" onClick={() => setMenuOpen(false)} className={`text-lg md:text-2xl ${location.pathname.startsWith("/documentos") ? "text-indigo-700 font-bold text-xl" : "text-gray-700 font-semibold"}`}>
                                <IoDocuments className="inline-block mr-1" /> Documentos
                            </Link>
                        </li>
                        <li>
                            <Link to="/administrador" onClick={() => setMenuOpen(false)} className={`text-lg md:text-2xl ${location.pathname.startsWith("/administrador") ? "text-indigo-700 font-bold text-xl" : "text-gray-700 font-semibold"}`}>
